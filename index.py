...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    coin_list = get_crypto_list()
    random_coins = random.sample(coin_list, 10)
    coins_names = '\n'.join([coin['name'] for coin in random_coins])
    await update.message.reply_text(
        f'Привіт! Введіть назву криптовалюти, щоб отримати її поточну ціну.\n'
        f'наприклад ви можете дізнатись вартість таких криптовалют: {coins_names}.'
    )
